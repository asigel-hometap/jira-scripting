...
# JQL Query for HT projects
JQL_QUERY = 'project = HT AND status NOT IN ("Won\'t Do") ORDER BY updated DESC'

# Status mappings for cycle time tracking
DISCOVERY_STATUSES = ['Generative Discovery', 'Problem Discovery', 'Solution Discovery']
BUILD_STATUSES = ['Build']
COMPLETION_STATUSES = ['Beta', 'Live']
HOLD_STATUSES = ['Inbox', 'Committed']
...
def calculate_cycle_times(projects: List[Dict[str, Any]], snapshot_date: str) -> List[Dict[str, Any]]:
    """Calculate cycle times for each project based on historical data."""
    logger.info("Calculating cycle times...")
    
    # Load historical snapshots for cycle time calculation
    historical_data = load_historical_snapshots()
    
    for project in projects:
        project_key = project['project_key']
        project['cycle_tracking'] = calculate_project_cycle_times(
            project_key, 
            historical_data, 
            snapshot_date
        )
...
def main():
    ...
    try:
        ...
        projects_with_cycles = calculate_cycle_times(projects, snapshot_date)
