...
def get_historical_assignee_changes():
    """Extract historical assignee changes from Jira changelog."""
    ...
    # Dictionary to store weekly snapshots
    ...
    for i, issue in enumerate(issues):
        ...
        if not created_str:
            # Fallback to issue.created if fields.created doesn't work
        ...
        # If no changelog, use current values
        if not issue.changelog:
            current_assignee = issue.fields.assignee.displayName if issue.fields.assignee else None
            if hasattr(issue.fields, 'customfield_10238') and issue.fields.customfield_10238:
                current_health = issue.fields.customfield_10238.value if hasattr(issue.fields.customfield_10238, 'value') else str(issue.fields.customfield_10238)
        
        # Generate weekly snapshots from creation date to now
        ...
        while current_date <= datetime.now():
            ...
            # Only include if issue was active during this week
            if current_assignee and current_date <= datetime.now():
                # Update team stats
                team_stats = weekly_snapshots[week_key]['team_stats'][current_assignee]
                team_stats['total_issues'] += 1
                
                # Categorize health status
                if 'on track' in current_health.lower():
                    team_stats['on_track'] += 1
                    weekly_snapshots[week_key]['health_counts']['On Track'] += 1
                elif 'off track' in current_health.lower():
                    team_stats['off_track'] += 1
                    weekly_snapshots[week_key]['health_counts']['Off Track'] += 1
                elif 'at risk' in current_health.lower() or 'risk' in current_health.lower():
                    team_stats['at_risk'] += 1
                    weekly_snapshots[week_key]['health_counts']['At Risk'] += 1
                elif 'complete' in current_health.lower():
                    team_stats['complete'] += 1
                    weekly_snapshots[week_key]['health_counts']['Complete'] += 1
                elif 'on hold' in current_health.lower():
                    team_stats['on_hold'] += 1
                    weekly_snapshots[week_key]['health_counts']['On Hold'] += 1
                elif 'mystery' in current_health.lower():
                ...
                team_stats['status_breakdown'][current_status] += 1
                weekly_snapshots[week_key]['status_counts'][current_status] += 1
...
def main():
    ...
    print("Starting Jira Historical Analysis...")
    print(f"Analysis date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    try:
        # Extract historical data
        weekly_snapshots = get_historical_assignee_changes()
        ...
        print(f"\nYou can now use this historical data to see trends over time!")
