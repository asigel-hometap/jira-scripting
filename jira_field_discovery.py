...
def discover_jira_fields():
    """Discover available fields in Jira to find Health and other relevant fields."""
    jira = JIRA(
        server=JIRA_URL,
    ...
    jql = 'project = "HT" ORDER BY created DESC'
    issues = jira.search_issues(jql, maxResults=5)
    
    if not issues:
        print("No issues found. Trying broader search...")
    ...
    queries_to_test = [
        'project = "HT" AND issuetype = "Project"',
        'project = "HT" AND issuetype = "Epic"',
        'project = "HT" AND issuetype = "Story"',
        'project = "HT"',
        'project = "Hometap"',
    ]
...
def main():
    ...
    print("Starting Jira Field Discovery...")
    ...
    try:
        ...
        print(f"2. Update the field names in jira_weekly_analysis.py")
        print(f"3. Test with the correct JQL query for your projects")
